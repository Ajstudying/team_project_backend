...
class OrderService(private val database: Database) {
    ...
    fun createOrders(orderData: OrderCreateRequest, authProfile: AuthProfile): Long {
        ...
        transaction {
            try {
                ...
                // 5. 주문 재고 업데이트
                updateOrderStock(resultOrderItems)
    ...
    // 주문 Item(도서)정보 등록
    fun createOrderItem(req: OrderCreateRequest, orderId: Long): List<OrderStockRequest> {
    ...
    // 주문 재고 업데이트
    fun updateOrderStock(orderItemStock: List<OrderStockRequest>) {
        println("\n<<< OrderService createOrderAddress >>>")
        ...
        for (reqItem in orderItemStock) {
            ...
            val query = OrderStock.select {
                (OrderStock.itemId eq reqItem.itemId)
            }

            // 이미 해당 도서의 재고가 등록되어 있으면 기존 재고에 수량을 더한다
            if (query.count() > 0) {
                // SQL
                // UPDATE order_stock SET book_stock = book_stock + 2 WHERE item_id = 1;
                OrderStock.update({ OrderStock.itemId eq reqItem.itemId }) {
                    it[bookStock] = OrderStock.bookStock + reqItem.quantity.toInt()
                }
            } else {
                // SQL
                // INSERT INTO order_stock( book_stock, item_id, book_id) values (?, ?, (select id from books where item_id =?))
                OrderStock.insert {
                    it[OrderStock.bookStock] = reqItem.quantity
                    it[OrderStock.itemId] = reqItem.itemId
