...
class NewBookApiController {
    ...
    fun postForeignDataMyServer() : ResponseEntity<Any> {
        println("외국도서신간 db입력")

        val dataFromExternalAPI: List<BookDataResponse> = newBooksClient.foreignFetch().item
        ...
        val result = transaction {
            ...
            for (data in dataFromExternalAPI) {
                ...
                if (existingBook == null) {
                    val foreignBook = ForeignBooks.insert {
                        it[this.publisher] = data.publisher
                        it[this.title] = data.title
                        it[this.link] = data.link
                        it[this.author] = data.author
